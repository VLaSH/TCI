...
module ApplicationHelper
  ...
  def get_video_url
    ...
    if setting && setting.method == '1'
      url = "http://player.vimeo.com/video/#{setting.value}"
    elsif setting && setting.method == '2'
      url = "http://www.youtube.com/embed/#{setting.value}"
    else
      ''
    end
  end
    ...
    if !attachment.image? && !attachment.video?
      ...
      if attachment.vimeo_video_id.present? and !options[:style] == :w60xh60
        ...
        if options[:lightview]
          target = case
            when attachment.vimeo_video_id.present? then "http://player.vimeo.com/video/#{ attachment.vimeo_video_id}"
            when attachment.youtube_video_id.present? then "http://www.youtube.com/embed/#{attachment.youtube_video_id}"
 ...
 def student_galleries(attachment, options = {})
  ...
  def get_attachment_url(attachment, style)
    if attachment.youtube_video_id.present?
      url = "http://www.youtube.com/embed/#{attachment.youtube_video_id}"
    elsif attachment.vimeo_video_id.present?
      url = "http://player.vimeo.com/video/#{attachment.vimeo_video_id}"
    else
