...
module PrettyButtons
  def pretty_button(label, options = {})
    image = options[:icon_path] ? image_tag(options[:icon_path]) : ''
    type  = options[:type]
  ...
  def pretty_button_link(name, options = {}, html_options = {})
    ...
    image = icon_path ? image_tag(icon_path) : ''
    link_to ((image + name).html_safe), options, html_options.merge(:class => "button #{type}")
  end
