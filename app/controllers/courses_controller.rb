...
include Theia::ActsAsDiscussable::Controller
  ...
  def index
    page_config do |page|
      ...
      if params.has_key?(:course_type)
        ...
        page.title.unshift('Online Photography Courses, Photography Classes Online, Photography Courses Online')
      end
    end
  ...
  def blogphoto
    head(:not_found) and return if (blog = Blog.find_by_id(params[:id])).nil?
    style = params[:style] || :original
    redirect_to(blog.photo_s3_url(style))
  end
  
  def fulldownload
	 @data = ActiveRecord::Base.connection.execute("SELECT course_id,id
										FROM `scheduled_courses`
										WHERE `id`
										IN ( SELECT scheduled_course_id
										FROM `scheduled_courses`
										INNER JOIN `enrolments` ON `enrolments`.`scheduled_course_id` = `scheduled_courses`.`id`
										AND `enrolments`.`deleted_at` IS NULL
										WHERE `enrolments`.`student_user_id` = #{current_user.id}
										AND `enrolments`.`unsubscribe` =0
										GROUP BY enrolments.scheduled_course_id, enrolments.student_user_id )")
		
	html = render_to_string(:action => :fulldownload, :layout => "pdf_layout.html") 
	pdf = WickedPdf.new.pdf_from_string(html) 
	send_data(pdf, :filename  => "course.pdf", :disposition => 'attachment') 	
