class Instructor::UpdatesController < Instructor::BaseController
  ...
  def index
    ...
    .joins("LEFT JOIN users u ON u.id = e.student_user_id").where("enrolment_id IN (#{allenrolments}) ")
    .order("assignment_submissions.updated_at DESC")
    #abort(submissions) //remove this AND instructor_unread = 1 AND completed = 1  
